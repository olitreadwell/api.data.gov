...
- 2026-09-25 self-found a11y gap — metrics page agency `<select>` (assets/javascripts/metrics/views/Metrics.js) had NO accessible name: no `<label for>`, no `aria-label`, no `aria-labelledby` -> WCAG 4.1.2 Name/Role/Value failure. Fix: added `aria-label="Select agency"` to the select. outcome: pr-opened (https://github.com/olitreadwell/api.data.gov/pull/4, branch fix/metrics-agency-select-accessibility). Deduped: no upstream issue/PR covers it. Verified locally: eslint --max-warnings 0 . clean, prettier --check clean; fork CI runs no substantive jobs (docker buildx --push to GSA packages) so results are locally-verified. No AI mention in body/commit.
